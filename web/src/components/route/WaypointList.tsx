  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="relative">
        <div className="flex items-center gap-1.5 rounded-md bg-zinc-800 border border-zinc-600 focus-within:border-blue-500 transition-colors">
          <span className="pl-2.5 text-zinc-500 text-sm">🔍</span>
          <input
            type="text"
            value={query}
          ...
          />
          {searching && (
            <span className="pr-2.5 text-[10px] text-zinc-500 animate-pulse">
              Searching...
            </span>
          )}
        </div>
        ...
        {showResults && results.length > 0 && (
          <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-zinc-800 border border-zinc-600 rounded-lg shadow-xl max-h-48 overflow-y-auto">
            {results.map((r, i) => (
              <button
                key={i}
                onClick={() => handleSelectResult(r)}
                className="w-full text-left px-3 py-2 text-sm text-zinc-300 hover:bg-zinc-700 transition-colors border-b border-zinc-700/50 last:border-0"
              >
        ...
        {showResults && !searching && results.length === 0 && query.trim().length >= 2 && (
          <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-zinc-800 border border-zinc-600 rounded-lg shadow-xl px-3 py-2">
            <span className="text-xs text-zinc-500">No results found</span>
          </div>
        )}
      </div>
          ...
          {waypoints.map((wp, i) => {
            ...
            return (
              <div
                ...
                className={`
                  flex items-center justify-between rounded-md bg-zinc-800 px-2 py-2 transition-all cursor-grab active:cursor-grabbing
              ...
              >
                ...
                <button
                  onClick={() => onRemove(i)}
                  className="text-zinc-500 hover:text-red-400 transition-colors text-sm shrink-0 ml-1"
                >
                  ×
                </button>
              </div>
